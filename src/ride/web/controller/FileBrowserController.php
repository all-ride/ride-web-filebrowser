...
namespace ride\web\controller;

use ride\library\i18n\I18n;

use ride\web\model\filebrowser\filter\PathFilter;
use ride\web\model\filebrowser\filter\Filter;
use ride\web\model\filebrowser\FileBrowser;

use ride\web\table\filebrowser\decorator\EditActionDecorator;
use ride\web\table\filebrowser\decorator\FileActionDecorator;
use ride\web\table\filebrowser\BrowserTable;
use ride\web\table\filebrowser\ClipboardTable;

use ride\web\FileBrowserModule;

use ride\library\archive\Archive;
...
class FileBrowserController extends AbstractController {
     ...
     * @param ride\web\model\filebrowser\filter\Filter $filter
     * @return null
     */
    ...
    public function archive(array $files = null) {
        ...
        $archiveFile = new File($this->ride->getApplicationDirectory(), ride::DIRECTORY_DATA . '/' . $name);
        $archiveFile = $archiveFile->getCopyFile();

        $archive = $this->ride->getDependency('ride\\library\\archive\\Archive', $archiveFile->getExtension(), array('file' => $archiveFile));
     ...
     * @param ride\library\system\file\File $path The current path
    ...
    private function getBrowserTable($action, File $path = null, $pathAction = null, $downloadAction = null, $renameAction = null, $editAction = null, $orderField = null, $orderDirection = null, $imageAction = null) {
        ...
        if(interface_exists('ride\\library\\archive\\Archive')) {
     ...
     * @param string $action URL to the action of the table form
     * @return ride\filebrowser\table\ClipboardTable
     */
     ...
     * @return ride\library\system\file\File The file object of the tokens
     */
